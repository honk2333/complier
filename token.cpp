...
int state_change(int state, char ch,MyWindow* ww) {   //状态转移函数,负数状态代表终止态,正数状态代表非终止态
    ...
    if (state == 8) {  //小数点后至少一位,指数后至少一位，然后跳转到状态7等待结束
        if (ch <= '9' && ch >= '0') {
            ...
            string dlgtitle="词法错误";
            string strinfo="执行过程发现词法错误,数据格式输入错误";
    ...
    if (state == 10) {   //等待读进右单引号
        if (ch == '\'')  //读进右单引号,等待读入下一字符,完成字符的输入
            ...
            string dlgtitle="词法错误";
            string strinfo="执行过程发现词法错误,字符格式错误";
            QMessageBox::warning(ww,QString::fromLocal8Bit(dlgtitle.c_str()),QString::fromLocal8Bit(strinfo.c_str()),QMessageBox::Ok);
            return 0;
            //printf("字符格式错误\n");
            //return 10;
    ...
    if (state == 12) {
        ...
        if (ch == '\"')  //读入右双引号.等待输入下一字符,结束字符串的输入
            ...
            string dlgtitle="词法错误";
            string strinfo="执行过程发现词法错误,字符串格式错误";
            QMessageBox::warning(ww,QString::fromLocal8Bit(dlgtitle.c_str()),QString::fromLocal8Bit(strinfo.c_str()),QMessageBox::Ok);
            return 17;
            //printf("字符串格式错误\n");
            //return 17;
...
void printword_list(MyWindow* w){
    ...
    for (int i = 0; i < len_a; i++) {
        ...
        ss=id+" "+ QString::fromLocal8Bit("a标志符 ")+name;
        w->print_wordlist(ss);
    }
    for (int i = 0; i < len_C; i++) {
        //cout << i << " " << "C字符 " << C[i] << endl;
        ...
        ss=id+" "+QString::fromLocal8Bit("C字符 ")+name;
        w->print_wordlist(ss);
    }
    for (int i = 0; i < len_s; i++) {
        ...
        ss=id+" "+QString::fromLocal8Bit("S字符串 ")+name;
        w->print_wordlist(ss);
    }
    for (int i = 0; i < len_c; i++) {
        //cout << i << " " << "c常数 " << c[i] << endl;
        QString id=QString::number(i);
        QString name=QString::fromStdString(c[i]);
        ss=id+" "+QString::fromLocal8Bit("c常数 ")+name;
        w->print_wordlist(ss);
    }
    for (int i = 0; i < len_k; i++) {
        ...
        ss=id+" "+QString::fromLocal8Bit("k关键字 ")+name;
        w->print_wordlist(ss);
    }
    for (int i = 0; i < len_p; i++) {
        //cout << i << " " << "p界符 " << p[i] << endl;
        ...
        ss=id+" "+QString::fromLocal8Bit("p界符 ")+name;
        w->print_wordlist(ss);
    }
}
...
void get_token(MyWindow* w) {
    ...
    while ((ch = getchar()) != '#') {
        ...
        if (state == 1 && word == "" &&
            ...
            word += ch; //右括号单独算一个单词
            //cout << word << endl;
            token[size_token].group = 'p';    //不用加符号表
